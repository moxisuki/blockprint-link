...
BG2 被检测到时,每次上传成功后会推送一条游戏内聊天通知,消息按蓝图类型区分:

```
§a[BlockPrint] §fLitematica 蓝图已接收: §ehouse.litematic
§a[BlockPrint] §fWorldEdit 蓝图已接收: §eigloo.schem
§a[BlockPrint] §fNBT蓝图已接收: §etemplate.nbt
§a[BlockPrint] §f建筑小帮手蓝图已接收: §etemplate.json §7— §a§n[点此复制]
```

- **检测**:运行时 classpath 探测 `com.direwolf20.buildinggadgets2.BuildingGadgets2` + FMLLoadCompleteEvent 二次确认。不读取版本号,不引入依赖。
- **路由**:与 WorldEdit 不同,BG2 上传**不**改变落盘目录(仍按 §2.3 路由)。
- **点击复制**(仅 BG2 `.json`):点击 `[点此复制]` → 一键复制完整 JSON 到剪贴板,同时聊天回执字节数。
- **降级**:非 `.json` 文件或无 BG2 时,仅显示纯文本通知。
