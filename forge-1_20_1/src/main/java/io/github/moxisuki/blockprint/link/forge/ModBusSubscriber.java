...
import io.github.moxisuki.blockprint.link.bridge.ui.UiHooks;
...
/**
 * MOD bus subscriber for Forge 1.20.1.
 * Uses the 1.20.1 KeyMapping constructor (String, IKeyConflictContext,
 * KeyModifier, Type, int, String category).
 */
...
public final class ModBusSubscriber {
    ...
    static void onRegisterOverlays(RegisterGuiOverlaysEvent event) {
        // 1.20.1 IGuiOverlay: (ForgeGui, GuiGraphics, float, int, int)
        event.registerAboveAll("blockprintlink_qr",
            (gui, guiGraphics, partialTick, screenWidth, screenHeight) ->
                UiHooks.onRenderHud(guiGraphics));
    }
}
